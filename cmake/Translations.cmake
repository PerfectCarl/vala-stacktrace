...
# File version  : 1.0
# File History:
#    - 0.1 : fixes for elementary.cmake

macro (add_translations_directory PO_SOURCE_DIR NLS_PACKAGE)
    ...
    foreach (LANGUAGE_NEEDED ${LANGUAGES_NEEDED})
        create_po_file (${PO_SOURCE_DIR} ${LANGUAGE_NEEDED})
    endforeach (LANGUAGE_NEEDED ${LANGUAGES_NEEDED})
    # generate .mo from .po
    file (GLOB PO_FILES ${PO_SOURCE_DIR}/*.po)
    file (RELATIVE_PATH OUTPUT_PATH "${CMAKE_CURRENT_SOURCE_DIR}" "${PO_SOURCE_DIR}")
    file(MAKE_DIRECTORY  ${CMAKE_CURRENT_BINARY_DIR}/${OUTPUT_PATH})
    foreach (PO_INPUT ${PO_FILES})
        get_filename_component (PO_INPUT_BASE ${PO_INPUT} NAME_WE)
        set (MO_OUTPUT ${CMAKE_CURRENT_BINARY_DIR}/${OUTPUT_PATH}/${PO_INPUT_BASE}.mo)
...
macro (create_po_file PO_SOURCE_DIR LANGUAGE_NEEDED)
    set (FILE ${PO_SOURCE_DIR}/${LANGUAGE_NEEDED}.po)
    if (NOT EXISTS ${PO_SOURCE_DIR}/${LANGUAGE_NEEDED}.po)
...
macro (add_translations_catalog PO_SOURCE_DIR NLS_PACKAGE)
    ...
    foreach(FILES_INPUT ${ARGN})
        set(BASE_DIRECTORY ${PO_SOURCE_DIR}/${FILES_INPUT})

        file (GLOB_RECURSE SOURCE_FILES RELATIVE ${PO_SOURCE_DIR}/ ${BASE_DIRECTORY}/*.c)
        foreach(C_FILE ${SOURCE_FILES})
            set(C_SOURCE ${C_SOURCE} ${C_FILE})
        endforeach()

        file (GLOB_RECURSE SOURCE_FILES RELATIVE ${PO_SOURCE_DIR}/ ${BASE_DIRECTORY}/*.vala)
        foreach(VALA_C_FILE ${SOURCE_FILES})
            set(VALA_SOURCE ${VALA_SOURCE} ${VALA_C_FILE})
        endforeach()

        file (GLOB_RECURSE SOURCE_FILES RELATIVE ${PO_SOURCE_DIR}/ ${BASE_DIRECTORY}/*.ui)
    ...
    set(BASE_XGETTEXT_COMMAND
        ...
        -o ${PO_SOURCE_DIR}/${NLS_PACKAGE}.pot
   ...
   set(CONTINUE_FLAG "")
    ...
    IF(NOT "${C_SOURCE}" STREQUAL "")
        add_custom_command(TARGET pot WORKING_DIRECTORY ${PO_SOURCE_DIR} COMMAND ${BASE_XGETTEXT_COMMAND} ${C_SOURCE})
        set(CONTINUE_FLAG "-j")
    ENDIF()

    IF(NOT "${VALA_SOURCE}" STREQUAL "")
        add_custom_command(TARGET pot WORKING_DIRECTORY ${PO_SOURCE_DIR} COMMAND ${BASE_XGETTEXT_COMMAND} ${CONTINUE_FLAG} -LC\# ${VALA_SOURCE})
        set(CONTINUE_FLAG "-j")
    ENDIF()

    IF(NOT "${GLADE_SOURCE}" STREQUAL "")
        add_custom_command (TARGET pot WORKING_DIRECTORY ${PO_SOURCE_DIR} COMMAND ${BASE_XGETTEXT_COMMAND} ${CONTINUE_FLAG} -LGlade ${GLADE_SOURCE})
    ENDIF()
endmacro ()
